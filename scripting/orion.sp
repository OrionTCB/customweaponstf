...
new bool:m_bLifeBreak_ATTRIBUTE[2049];
new Float:m_flLifeBreak_HP[2049]                    = {0.0, ...};
...
new bool:m_bBladeStorm_ATTRIBUTE[2049];
new m_iBladeStorm_Damage[2049]              = {0, ...};
new Float:m_flBladeStorm_Radius[2049]       = {0.0, ...};
new Float:m_flBladeStorm_Interval[2049]     = {0.0, ...};
new Float:m_flBladeStorm_Duration[2049]     = {0.0, ...};
new m_iBladeStorm_NOA[2049]                 = {0, ...};
new m_iBladeStorm_Attack[2049]              = {0, ...};
new Float:m_flBladeStorm_Cooldown[2049]     = {0.0, ...};
...
public Action:CustomWeaponsTF_OnAddAttribute( m_iWeapon, m_iClient, const String:m_sAttribute[], const String:m_sPlugin[], const String:m_sValue[] )
{
    ...
    {
        m_flMetalOnHit_Amount[m_iWeapon]     = StringToInt( m_sValue );
    ...
    else if ( StrEqual( m_sAttribute, "blade storm" ) )
    {
        ...
        m_iBladeStorm_Damage[m_iWeapon]     = StringToInt( m_sValues[0] );
        m_flBladeStorm_Radius[m_iWeapon]    = StringToFloat( m_sValues[1] );
        m_flBladeStorm_Interval[m_iWeapon]  = StringToFloat( m_sValues[2] );
        m_flBladeStorm_Duration[m_iWeapon]  = StringToFloat( m_sValues[3] );
        m_iBladeStorm_NOA[m_iWeapon]        = StringToInt( m_sValues[4] );
        m_iBladeStorm_Attack[m_iWeapon]     = StringToInt( m_sValues[5] );
        m_flBladeStorm_Cooldown[m_iWeapon]  = StringToFloat( m_sValues[6] );
        m_bBladeStorm_ATTRIBUTE[m_iWeapon]  = true;
        m_aAction = Plugin_Handled;
    }
    /* Chance To Mad Milk
     *
    ...
    {
        m_flLifeBreak_HP[m_iWeapon]         = StringToFloat( m_sValue );
        m_bLifeBreak_ATTRIBUTE[m_iWeapon]   = true;
        m_aAction = Plugin_Handled;
    }
                        ...
                        //-//
                            ...
                            if ( m_bBackstabDamageModSubStun_ATTRIBUTE[m_iWeapon] && m_iCustom == TF_CUSTOM_BACKSTAB )
                            {
                                if ( TF2_GetPlayerClass( m_iAttacker ) == TFClass_Spy )
                                {
                                    ...
                                    m_flDamage = RoundToCeil( GetMax( TF2_GetClientMaxHealth( m_iVictim ), GetClientHealth( m_iVictim ) ) * m_flBackstabDamageModSubStun_Multiplier[m_iWeapon] ) / 3.0;
...
public Action:OnTakeDamageAlive( m_iVictim, &m_iAttacker, &m_iInflictor, &Float:m_flDamage, &m_iType, &m_iWeapon, Float:m_flForce[3], Float:m_flPosition[3], m_iCustom )
{
        ...
        && !HasInvulnerabilityCond( m_iVictim ) )
    {
        ...
        //-//
            if ( m_bLifeBreak_ATTRIBUTE[m_iWeapon] )
            {
                if ( TF2_GetClientMaxHealth( m_iVictim ) >= 1.0 )
                {
                    m_iIntegers[m_iVictim][m_iLifeBreak_Stack]++;
                    TF2Attrib_SetByName( m_iVictim, "max health additive penalty", ( m_flLifeBreak_HP[m_iWeapon] * m_iIntegers[m_iVictim][m_iLifeBreak_Stack] ) ); 
                }
            }
            ...
            //-//
                if ( m_bUberchargeOnHit_ATTRIBUTE[m_iWeapon] && TF2_GetPlayerClass( m_iAttacker ) == TFClass_Medic )
                    TF2_SetPlayerUberLevel( m_iAttacker, TF2_GetPlayerUberLevel( m_iAttacker ) + m_flUberchargeOnHit_Amount[m_iWeapon] );
            //-//
...
public OnEntityDestroyed( m_iEntity )
{
    ...
    m_bLifeBreak_ATTRIBUTE[m_iEntity]                   = false;
    m_flLifeBreak_HP[m_iEntity]                         = 0.0;
    ...
    m_bBladeStorm_ATTRIBUTE[m_iEntity]      = false;
    m_iBladeStorm_Damage[m_iEntity]         = 0;
    m_flBladeStorm_Radius[m_iEntity]        = 0.0;
    m_flBladeStorm_Interval[m_iEntity]      = 0.0;
    m_flBladeStorm_Duration[m_iEntity]      = 0.0;
    m_iBladeStorm_NOA[m_iEntity]            = 0;
    m_iBladeStorm_Attack[m_iEntity]         = 0;
    m_flBladeStorm_Cooldown[m_iEntity]      = 0.0;
