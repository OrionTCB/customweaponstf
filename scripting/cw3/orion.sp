...
enum
{
    ...
    Handle:m_hBladeStorm_TimerCooldown,
    Handle:m_hBladeStorm_TimerDuration,
    Handle:m_hBladeStorm_TimerInterval,
...
new bool:m_bLifeBreak_ATTRIBUTE[MAXPLAYERS + 1][MAXSLOTS + 1];
new Float:m_flLifeBreak_HP[MAXPLAYERS + 1][MAXSLOTS + 1];
...
new m_iMetalDrain_PassiveOrActive[MAXPLAYERS + 1][MAXSLOTS + 1];
...
new m_iDamageReceivedUnleashedDeath_PassiveOrActive[MAXPLAYERS + 1][MAXSLOTS + 1];
...
new bool:m_bBladeStorm_ATTRIBUTE[MAXPLAYERS + 1][MAXSLOTS + 1];
new m_iBladeStorm_Damage[MAXPLAYERS + 1][MAXSLOTS + 1];
new Float:m_flBladeStorm_Radius[MAXPLAYERS + 1][MAXSLOTS + 1];
new Float:m_flBladeStorm_Interval[MAXPLAYERS + 1][MAXSLOTS + 1];
new Float:m_flBladeStorm_Duration[MAXPLAYERS + 1][MAXSLOTS + 1];
new m_iBladeStorm_NOA[MAXPLAYERS + 1][MAXSLOTS + 1];
new m_iBladeStorm_Attack[MAXPLAYERS + 1][MAXSLOTS + 1];
new Float:m_flBladeStorm_Cooldown[MAXPLAYERS + 1][MAXSLOTS + 1];
...
public Event_PostInventoryApplication( Handle:m_hEvent, const String:m_strName[], bool:m_bDontBroadcast )
{
    ...
    if ( IsValidClient( m_iClient ) && IsPlayerAlive( m_iClient ) )
    {
        ...
        if ( m_hTimers[m_iClient][m_hBladeStorm_TimerDuration] != INVALID_HANDLE )
        {
            ClearTimer( m_hTimers[m_iClient][m_hBerserker_TimerDuration] );
            m_iIntegers[m_iClient][m_iBladeStorm_Stack] = 0;
        }
        if ( m_hTimers[m_iClient][m_hBladeStorm_TimerCooldown] != INVALID_HANDLE )
        {
            ClearTimer( m_hTimers[m_iClient][m_hBladeStorm_TimerCooldown] );
            PrintHintText( m_iClient, "Custom: Blade Storm is ready." );
...
public OnPreThink( m_iClient )
{
    ...
    new m_iSlot = GetClientSlot( m_iClient );
    ...
    for ( m_iSlot2 = 0; m_iSlot2 <= 4; m_iSlot2++ ) // ALWAYS ACTIVE | PASSIVE STUFF HERE.
    {
        ...
        m_iButtons = ATTRIBUTE_BLADESTORM( m_iClient, m_iButtons, m_iSlot2, m_iButtonsLast );
...
ATTRIBUTE_HEATFIRERATE( m_iClient, &m_iButtons, &m_iSlot, &m_iButtonsLast )
{
    if ( HasAttribute( m_iClient, _, m_bHeatFireRate_ATTRIBUTE, true ) )
    {
        ...
        new ammo = GetAmmo( m_iClient, GetClientSlot( m_iClient ) );
...
ATTRIBUTE_DISABLEUBER( m_iClient, &m_iButtons, &m_iSlot, &m_iButtonsLast )
{
    ...
    {
        if ( TF2_GetPlayerUberLevel( m_iClient ) >= 99.0 ) TF2_SetPlayerUberLevel( m_iClient, 99.0 );
...
{
    if ( HasAttribute( m_iClient, _, m_bSetWeaponSwitch_ATTRIBUTE ) ) SetClientSlot( m_iClient, GetAttributeValue( m_iClient, _, m_bSetWeaponSwitch_ATTRIBUTE, m_iSetWeaponSwith_Slot ) );

    return m_iButtons;
}

ATTRIBUTE_BLADESTORM( m_iClient, &m_iButtons, &m_iSlot, &m_iButtonsLast )
{
    if ( HasAttribute( m_iClient, 2, m_bBladeStorm_ATTRIBUTE, true ) )
    {
        new Float:radius = GetAttributeValue( m_iClient, 2, m_bBladeStorm_ATTRIBUTE, m_flBladeStorm_Radius, true );
        new Float:duration = GetAttributeValue( m_iClient, 2, m_bBladeStorm_ATTRIBUTE, m_flBladeStorm_Duration, true );
        new Float:cooldown = GetAttributeValue( m_iClient, 2, m_bBladeStorm_ATTRIBUTE, m_flBladeStorm_Cooldown, true );
        new noa = GetAttributeValue( m_iClient, 2, m_bBladeStorm_ATTRIBUTE, m_iBladeStorm_NOA, true );
        new Float:interval = GetAttributeValue( m_iClient, 2, m_bBladeStorm_ATTRIBUTE, m_flBladeStorm_Interval, true );
        new damage = GetAttributeValue( m_iClient, 2, m_bBladeStorm_ATTRIBUTE, m_iBladeStorm_Damage, true );
        new attack = GetAttributeValue( m_iClient, 2, m_bBladeStorm_ATTRIBUTE, m_iBladeStorm_Attack, true );

        new bool:IsTargetNear[MAXPLAYERS + 1] = false;
        new target;
        ...
        if ( m_hTimers[m_iClient][m_hBladeStorm_TimerCooldown] == INVALID_HANDLE )
        {
            for ( new clients = 1; clients <= MaxClients; clients++ )
            {
                if ( clients != m_iClient && IsValidClient( clients ) && IsPlayerAlive( clients ) && GetClientTeam( clients ) != GetClientTeam( m_iClient ) )
                {
                    if ( !TF2_IsPlayerInCondition( clients, TFCond_Cloaked ) && !TF2_IsPlayerInCondition( clients, TFCond_Stealthed ) && !HasInvulnerabilityCond( clients ) )
                    {
                        new Float:m_flPos1[3];
                        GetClientAbsOrigin( m_iClient, m_flPos1 );
                        new Float:m_flPos2[3];
                        GetClientAbsOrigin( clients, m_flPos2 );

                        new Float:m_flDistance = GetVectorDistance( m_flPos1, m_flPos2 );
                        if ( m_flDistance <= radius && m_iButtons & IN_RELOAD == IN_RELOAD )
                        {
                            m_hTimers[m_iClient][m_hBladeStorm_TimerCooldown] = CreateTimer( cooldown, m_tBladeStorm_TimerCooldown, m_iClient );
                            if ( m_hTimers[m_iClient][m_hBladeStorm_TimerDuration] != INVALID_HANDLE ) ClearTimer( m_hTimers[m_iClient][m_hBladeStorm_TimerDuration] );
                            m_hTimers[m_iClient][m_hBladeStorm_TimerDuration] = CreateTimer( duration, m_tBladeStorm_TimerDuration, m_iClient );
                        }
                    }
                }
            }
        ...
        if ( m_hTimers[m_iClient][m_hBladeStorm_TimerDuration] != INVALID_HANDLE )
        {
            if ( IsValidClient( m_iClient ) )
            {
                for ( new clients = 1; clients <= MaxClients; clients++ )
                {
                    // Not the player, is valid, is alive, not on the same team.
                    if ( clients != m_iClient && IsValidClient( clients ) && IsPlayerAlive( clients ) && GetClientTeam( clients ) != GetClientTeam( m_iClient ) )
                    {
                        // Not invis, not invulnerable.
                        if ( !TF2_IsPlayerInCondition( clients, TFCond_Cloaked )
                            && !TF2_IsPlayerInCondition( m_iClient, TFCond_Cloaked )
                            && !TF2_IsPlayerInCondition( clients, TFCond_Stealthed )
                            && !TF2_IsPlayerInCondition( m_iClient, TFCond_Stealthed )
                            && !HasInvulnerabilityCond( clients ) )
                        {
                            new Float:m_flPos1[3];
                            GetClientAbsOrigin( m_iClient, m_flPos1 );
                            new Float:m_flPos2[3];
                            ...
                            new Float:m_flDistance = GetVectorDistance( m_flPos1, m_flPos2 );
                            if ( m_flDistance <= radius )
                            {
                                // RECHECKING : Target is I, target is not the player, is valid, is alive, is not on the same team as the client.
                                target = GetRandomInt( 1, MaxClients );
                                if ( target == clients && target != m_iClient && IsValidClient( target ) && IsPlayerAlive( target ) && GetClientTeam( target ) != GetClientTeam( m_iClient )  && m_iIntegers[m_iClient][m_iBladeStorm_Stack] <= noa )
                                {
                                    // Get victim's eyes' angles.
                                    new Float:victim_angle[3];
                                    GetClientEyeAngles( target, victim_angle );

                                    // Determine the pos xyz to be on when teleporting.
                                    new Float:victim_angle_rad = victim_angle[1] * PI / 180.0;
                                    new Float:attacker_new[3];
                                    attacker_new[0] = m_flPos2[0] - 55 * Cosine( victim_angle_rad );
                                    attacker_new[1] = m_flPos2[1] - 55 * Sine( victim_angle_rad );
                                    attacker_new[2] = m_flPos2[2];

                                    // Get infos.
                                    new Float:attacker_min[3], Float:attacker_max[3];
                                    GetEntPropVector( m_iClient, Prop_Send, "m_vecMins", attacker_min );
                                    GetEntPropVector( m_iClient, Prop_Send, "m_vecMaxs", attacker_max );

                                    // Predicting the futur here.
                                    if ( !WillEntityGetStuck( m_iClient, attacker_new, attacker_min, attacker_max ))
                                    {
                                        IsTargetNear[target] = true;
                                        if ( m_hTimers[m_iClient][m_hBladeStorm_TimerInterval] == INVALID_HANDLE && IsTargetNear[target] )
                                        {
                                            if ( noa > 0 ) m_iIntegers[m_iClient][m_iBladeStorm_Stack]++;

                                            // Create Interval timer.
                                            m_hTimers[m_iClient][m_hBladeStorm_TimerInterval] = CreateTimer( interval, m_tBladeStorm_Timer, m_iClient );

                                            // Set up, create particle.
                                            new m_pParticle;

                                            m_pParticle = CreateEntityParticle( TF2_GetClientTeam( m_iClient ) == TFTeam_Red ? "spy_stolen_smoke_red" : "spy_stolen_smoke_blue", m_flPos1 );
                                            SetEntitySelfDestruct( m_pParticle, true );
                                            m_pParticle = CreateEntityParticle( TF2_GetClientTeam( m_iClient ) == TFTeam_Red ? "spy_stolen_smoke_red" : "spy_stolen_smoke_blue", m_flPos2 );
                                            SetEntitySelfDestruct( m_pParticle, true );

                                            m_pParticle = CreateEntityParticle( TF2_GetClientTeam( m_iClient ) == TFTeam_Red ? "spy_stolen_flash_red" : "spy_stolen_flash_blue", m_flPos1 );
                                            SetEntitySelfDestruct( m_pParticle, true );
                                            m_pParticle = CreateEntityParticle( TF2_GetClientTeam( m_iClient ) == TFTeam_Red ? "spy_stolen_flash_red" : "spy_stolen_flash_blue", m_flPos2 );
                                            SetEntitySelfDestruct( m_pParticle, true );

                                            // Add condition.
                                            TF2_AddCondition( m_iClient, TFCond_MegaHeal,       interval );
                                            TF2_AddCondition( m_iClient, TFCond_SpeedBuffAlly,  interval );
                                            TF2_AddCondition( m_iClient, TFCond_TeleportedGlow, interval );

                                            // Reduce player alpha.
                                            SetEntityRenderMode( m_iClient, RENDER_TRANSCOLOR );
                                            SetEntityRenderColor( m_iClient, 255, 215, 0, 128 );

                                            // Play sounds.
                                            // EmitSoundToClient( m_iClient, SOUND_RADIANCE );
                                            EmitSoundToClient( target, SOUND_RADIANCE );

                                            // Teleport player to victim.
                                            TeleportEntity( m_iClient, attacker_new, victim_angle, NULL_VECTOR );

                                            // Deal damage, stun and make the player attack ( if on ).
                                            DealDamage( target, damage, m_iClient, TF_DMG_PREVENT_PHYSICS_FORCE|DOTA_DMG_OTHER, "firedeath" );
                                            TF2_StunPlayer( target, 0.01, 1.0, TF_STUNFLAG_BONKSTUCK|TF_STUNFLAG_NOSOUNDOREFFECT, m_iClient );
                                            if ( attack > 0 )
                                            {
                                                new ent = GetPlayerWeaponSlot( m_iClient, 2 );
                                                if ( ent > 0 && IsValidEntity( ent ) && IsValidEdict( ent ) )
                                                {
                                                    TF2Attrib_SetByName( ent, "fire rate bonus", 0.01 );
                                                    m_iButtons |= IN_ATTACK;
                                                }
                                            }
                                        }
                                    } else {
                                        IsTargetNear[target] = false;
                                    }
                                } else {
                                    IsTargetNear[target] = false;
                                }
            ...
            new ent = GetPlayerWeaponSlot( m_iClient, 2 );
            if ( ent > 0 && IsValidEntity( ent ) && IsValidEdict( ent ) ) {
                if ( m_hTimers[m_iClient][m_hBladeStorm_TimerInterval] == INVALID_HANDLE || !IsTargetNear[target] ) TF2Attrib_SetByName( ent, "fire rate bonus", 1.0 );
            }
        }
        /* *** */
    }
...
/*stock Reroll( old_bad_target, target )
{
    target = GetRandomInt( 0, MaxClients );
    if ( IsValidClient( target ) && IsValidClient( old_bad_target ) ) {
        if ( target == old_bad_target ) Reroll( old_bad_target, target );
        else return target;
    }
...
PRETHINK_AFTERBURN( m_iClient, &m_iButtons, &m_iSlot, &m_iButtonsLast )
{
    ...
    if ( IsValidClient( m_iClient ) && m_iIntegers[m_iClient][m_iLifeBreak_Stack] < 1 ) {
        TF2Attrib_SetByName( m_iClient, "max health additive penalty", 0.0 );
    }
...
public Action:CW3_OnAddAttribute( m_iSlot, m_iClient, const String:m_sAttribute[], const String:m_sPlugin[], const String:m_sValue[], bool:m_bActive )
{
    ...
    else if ( StrEqual( m_sAttribute, "dmg taken dmg nearby enemies on death" ) )
    {
        ...
        m_iDamageReceivedUnleashedDeath_PassiveOrActive[m_iClient][m_iSlot]         = StringToInt( m_sValues[2] );
    ...
    else if ( StrEqual( m_sAttribute, "metal drain" ) )
    {
        ...
        m_iMetalDrain_PassiveOrActive[m_iClient][m_iSlot]    = StringToInt( m_sValues[2] );
    ...
    {
        m_flMetalOnHit_Amount[m_iClient][m_iSlot]     = StringToInt( m_sValue );
     ...
     *
     * ---------------------------------------------------------------------- */
    else if ( StrEqual( m_sAttribute, "blade storm" ) )
    {
        new String:m_sValues[7][10];
        ExplodeString( m_sValue, " ", m_sValues, sizeof( m_sValues ), sizeof( m_sValues[] ) );

        m_iBladeStorm_Damage[m_iClient][m_iSlot]     = StringToInt( m_sValues[0] );
        m_flBladeStorm_Radius[m_iClient][m_iSlot]    = StringToFloat( m_sValues[1] );
        m_flBladeStorm_Interval[m_iClient][m_iSlot]  = StringToFloat( m_sValues[2] );
        m_flBladeStorm_Duration[m_iClient][m_iSlot]  = StringToFloat( m_sValues[3] );
        m_iBladeStorm_NOA[m_iClient][m_iSlot]        = StringToInt( m_sValues[4] );
        m_iBladeStorm_Attack[m_iClient][m_iSlot]     = StringToInt( m_sValues[5] );
        m_flBladeStorm_Cooldown[m_iClient][m_iSlot]  = StringToFloat( m_sValues[6] );
        m_bBladeStorm_ATTRIBUTE[m_iClient][m_iSlot]  = true;
        m_aAction = Plugin_Handled;
    }
    /* Chance To Mad Milk
     *
     ...
     *
     * ---------------------------------------------------------------------- */
    else if ( StrEqual( m_sAttribute, "life break" ) )
    {
        m_flLifeBreak_HP[m_iClient][m_iSlot]         = StringToFloat( m_sValue );
        m_bLifeBreak_ATTRIBUTE[m_iClient][m_iSlot]   = true;
        m_aAction = Plugin_Handled;
    }
...
public CW3_OnWeaponRemoved( m_iSlot, m_iClient )
{
    if ( IsValidClient( m_iClient ) )
    {
        ...
        m_bLifeBreak_ATTRIBUTE[m_iClient][m_iSlot]                   = false;
        m_flLifeBreak_HP[m_iClient][m_iSlot]                         = 0.0;
        ...
        m_iMetalDrain_PassiveOrActive[m_iClient][m_iSlot]            = 0;
        ...
        m_iDamageReceivedUnleashedDeath_PassiveOrActive[m_iClient][m_iSlot]  = 0;
        ...
        m_bBladeStorm_ATTRIBUTE[m_iClient][m_iSlot]      = false;
        m_iBladeStorm_Damage[m_iClient][m_iSlot]         = 0;
        m_flBladeStorm_Radius[m_iClient][m_iSlot]        = 0.0;
        m_flBladeStorm_Interval[m_iClient][m_iSlot]      = 0.0;
        m_flBladeStorm_Duration[m_iClient][m_iSlot]      = 0.0;
        m_iBladeStorm_NOA[m_iClient][m_iSlot]            = 0;
        m_iBladeStorm_Attack[m_iClient][m_iSlot]         = 0;
        m_flBladeStorm_Cooldown[m_iClient][m_iSlot]      = 0.0;
    }
}
    ...
    if ( m_flDamage >= 1.0 )
    {
        ...
        //-//
            if ( HasAttribute( m_iVictim, _, m_bDamageResHealthMissing_ATTRIBUTE ) && ( GetAttributeValue( m_iVictim, _, m_bDamageResHealthMissing_ATTRIBUTE, m_flDamageResHealthMissing_ResPctPerMissingHpPct ) * GetAttributeValue( m_iVictim, _, m_bDamageResHealthMissing_ATTRIBUTE, m_iDamageResHealthMissing_MaxStackOfMissingHpPct ) ) >= 1.0 )
                m_flDamage = 0.0;
        //-//
            if ( HasAttribute( m_iVictim, 2, m_bBladeStorm_ATTRIBUTE, true ) && m_hTimers[m_iVictim][m_hBladeStorm_TimerInterval] != INVALID_HANDLE )
                m_flDamage = 0.0;
        //-//
                    ...
                    && m_iAttacker != m_iVictim )
                {
                    ...
                    if ( m_iWeapon != -1 )
                    {
                        ...
                        //-//
                            ...
                            if ( m_bBackstabDamageModSubStun_ATTRIBUTE[m_iAttacker][m_iSlot] && m_iCustom == TF_CUSTOM_BACKSTAB )
                            {
                                if ( TF2_GetPlayerClass( m_iAttacker ) == TFClass_Spy )
                                {
                                    ...
                                    m_flDamage = RoundToCeil( GetMax( TF2_GetClientMaxHealth( m_iVictim ), GetClientHealth( m_iVictim ) ) * m_flBackstabDamageModSubStun_Multiplier[m_iAttacker][m_iSlot] ) / 3.0;
                        ...
                        //-//
                             ...
                             *
                             * -------------------------------------------------- */
                            if ( m_bCritVsInvisiblePlayer_ATTRIBUTE[m_iAttacker][m_iSlot] )
                            {
                        ...
                        //-//
                            if ( m_bCritVictimInMidAir_ATTRIBUTE[m_iAttacker][m_iSlot] )
                            {
                                if ( !( GetEntityFlags( m_iVictim ) & FL_ONGROUND ) && !( GetEntityFlags( m_iVictim ) & FL_INWATER ) )
                                    m_iType = TF_DMG_CRIT|m_iType;
                            }
                        //-//
                            if ( m_bCritVictimScared_ATTRIBUTE[m_iAttacker][m_iSlot] )
                            {
                        ...
                        //-//
                            ...
                            if ( m_bCritVictimInWater_ATTRIBUTE[m_iAttacker][m_iSlot] )
                            {
                                if ( GetEntityFlags( m_iVictim ) & FL_INWATER )
                                    m_iType = TF_DMG_CRIT|m_iType;
                            }
                        ...
                        //-//
                            if ( m_bCritVsBurningCLOSERANGE_ATTRIBUTE[m_iAttacker][m_iSlot] )
                            {
                                new Float:m_flPos1[3], Float:m_flPos2[3];
                                GetClientAbsOrigin( m_iAttacker, m_flPos1 );
                                GetClientAbsOrigin( m_iVictim, m_flPos2 );
...
public Action:OnTakeDamageAlive( m_iVictim, &m_iAttacker, &m_iInflictor, &Float:m_flDamage, &m_iType, &m_iWeapon, Float:m_flForce[3], Float:m_flPosition[3], m_iCustom )
{
        ...
        && !HasInvulnerabilityCond( m_iVictim ) )
    {
        ...
        //-//
            if ( m_bLifeBreak_ATTRIBUTE[m_iAttacker][m_iSlot] )
            {
                if ( TF2_GetClientMaxHealth( m_iVictim ) >= 1.0 )
                {
                    m_iIntegers[m_iVictim][m_iLifeBreak_Stack]++;
                    TF2Attrib_SetByName( m_iVictim, "max health additive penalty", ( m_flLifeBreak_HP[m_iAttacker][m_iSlot] * m_iIntegers[m_iVictim][m_iLifeBreak_Stack] ) ); 
                }
            }
            ...
            if ( m_iVictim != m_iAttacker )
            {
                if ( m_bDrainUbercharge_ATTRIBUTE[m_iAttacker][m_iSlot] && TF2_GetPlayerClass( m_iVictim ) == TFClass_Medic && TF2_GetPlayerClass( m_iAttacker ) == TFClass_Medic )
                {
                    ...
                    new Float:m_flAttackerUbercharge = TF2_GetPlayerUberLevel( m_iAttacker );
                    new Float:m_flVictimUbercharge = TF2_GetPlayerUberLevel( m_iVictim );
                    ...
                    if ( m_flVictimUbercharge > 0.0 && m_flAttackerUbercharge < 100.0 )
                    {
                        if ( m_flVictimUbercharge >= ( pct * 100.0 ) )
                        {
                            if ( m_flAttackerUbercharge > ( 100.0 - ( pct * 100.0 ) ) )
                            {
                                ...
                                TF2_SetPlayerUberLevel( m_iVictim, m_flVictimUbercharge );

                                TF2_SetPlayerUberLevel( m_iAttacker, 100.0 );
                            } else {
                                m_flAttackerUbercharge += ( pct * 100.0 );
                                TF2_SetPlayerUberLevel( m_iAttacker, m_flAttackerUbercharge );

                                m_flVictimUbercharge -= ( pct * 100.0 );
                                TF2_SetPlayerUberLevel( m_iVictim, m_flVictimUbercharge );
                            }
                        } else {
                            TF2_SetPlayerUberLevel( m_iVictim, 0.0 );
                            TF2_SetPlayerUberLevel( m_iAttacker, ( m_flAttackerUbercharge + m_flVictimUbercharge ) );
                        }
                    }
                }
            //-//
                if ( m_bUberchargeOnHit_ATTRIBUTE[m_iAttacker][m_iSlot] && TF2_GetPlayerClass( m_iAttacker ) == TFClass_Medic )
                    TF2_SetPlayerUberLevel( m_iAttacker, TF2_GetPlayerUberLevel( m_iAttacker ) + m_flUberchargeOnHit_Amount[m_iAttacker][m_iSlot] );
            //-//
                if ( m_bMetalOnHit_ATTRIBUTE[m_iAttacker][m_iSlot] && TF2_GetPlayerClass( m_iAttacker ) == TFClass_Engineer )
                {
                    ...
                    TF2_SetClientMetal( m_iAttacker, TF2_GetClientMetal( m_iAttacker ) + ( metal < 1.0 ? 200.0 * metal : metal ) );
                }
            //-//
        ...
        && m_iWeapon != -1 )
    {
        ...
        //-//
            if ( m_bMetalPerShot_ATTRIBUTE[m_iClient][m_iSlot] && TF2_GetPlayerClass( m_iClient ) == TFClass_Engineer )
            {
                ...
                TF2_SetClientMetal( m_iClient, TF2_GetClientMetal( m_iClient ) + ( metal < 1.0 ? 200.0 * metal : metal ) );
            }
        //-//
...
public Action:Event_Death( Handle:m_hEvent, const String:m_strName[], bool:m_bDontBroadcast )
{
    ...
    if ( IsValidClient( m_iVictim ) )
    {
        ...
        //-//
            if ( HasAttribute( m_iVictim, _, m_bAttackSpeedOnKill_ATTRIBUTE ) )
            {
                if ( GetAttributeValue( m_iVictim, _, m_bAttackSpeedOnKill_ATTRIBUTE, m_flAttackSpeedOnKill_Removal ) < 1.0 )
                    m_iIntegers[m_iVictim][m_iAttackSpeed] = RoundToFloor( m_iIntegers[m_iVictim][m_iAttackSpeed] * GetAttributeValue( m_iVictim, _, m_bAttackSpeedOnKill_ATTRIBUTE, m_flAttackSpeedOnKill_Removal ) );
            }
            ...
            && m_iKiller != m_iVictim )
        {
            ...
            {
                if ( m_bKillGib_ATTRIBUTE[m_iKiller][m_iSlot] ) // Thx Thrawn2.
                {
...
public TF2_OnConditionRemoved( m_iClient, TFCond:condition )
{
    if ( IsValidClient( m_iClient ) )
    {
        if ( m_iIntegers[m_iVictim][m_iHotSauceType] != 0 )
        {
            new type = m_iIntegers[m_iVictim][m_iHotSauceType];

            if ( type == 1 || type == 4 || type == 5 || type == 7 )
                if ( condition == TFCond_Milked ) m_iIntegers[m_iVictim][m_iHotSauceType] = 0;
            if ( type == 2 || type == 4 || type == 6 || type == 7 )
                if ( condition == TFCond_Jarated ) m_iIntegers[m_iVictim][m_iHotSauceType] = 0;
            if ( type == 3 || type == 5 || type == 6 || type == 7 )
                if ( condition == TFCond_Bleeding ) m_iIntegers[m_iVictim][m_iHotSauceType] = 0;
        }
...
public Action:m_tDrainMetal_TimerInterval( Handle:timer, any:m_iClient )
{
    if ( HasAttribute( m_iClient, _, m_bMetalDrain_ATTRIBUTE ) )
    {
        ...
        if ( TF2_GetPlayerClass( m_iClient ) == TFClass_Engineer )
            TF2_SetClientMetal( m_iClient, metal_p + ( metal < 1.0 ? metal_p * metal : metal ) );
    }
...
public Action:m_tDamageReceivedUnleashedDeath_TimerDelay( Handle:timer, any:m_iVictim )
{
    if ( HasAttribute( m_iVictim, _, m_bDamageReceivedUnleashedDeath_ATTRIBUTE ) )
    {
        ...
        for ( new clients = 1; clients <= MaxClients; clients++ )
        {
            if ( clients != m_iVictim && IsValidClient( clients ) && IsPlayerAlive( clients ) && GetClientTeam( clients ) != GetClientTeam( m_iVictim ) )
            {
                if ( !TF2_IsPlayerInCondition( clients, TFCond_Ubercharged ) )
                {
                    ...
                    new Float:distance = GetVectorDistance( m_flPos1, m_flPos2 );
                    new Float:final_radius = radius + ( m_iIntegers[m_iVictim][m_iDamageReceived] * 0.2 );
                    if ( distance < final_radius )
                    {
                        decl Handle:m_hSee;
                        ( m_hSee = INVALID_HANDLE );

                        m_hSee = TR_TraceRayFilterEx( m_flPos1, m_flPos2, MASK_SOLID, RayType_EndPoint, TraceEntityFilterPlayer, m_iVictim );
                        if ( m_hSee != INVALID_HANDLE )
                        {
                            if ( !TR_DidHit( m_hSee ) )
                            {
                                ...
                                if ( distance > 73.0 )
                                    dmg_reduction = ( m_iIntegers[m_iVictim][m_iDamageReceived] * ( final_radius - ( ( distance - 73.0 ) * 0.5 ) ) / final_radius ) / m_iIntegers[m_iVictim][m_iDamageReceived];

                                DealDamage( clients, RoundToFloor( m_iIntegers[m_iVictim][m_iDamageReceived] * dmg_reduction ), m_iVictim, TF_DMG_PREVENT_PHYSICS_FORCE|DOTA_DMG_BLADEMAIL, "pumpkindeath" );
                            }
                        }
        ...
        m_iIntegers[m_iVictim][m_iDamageReceived] = 0;
    }
}
...
public Action:m_tBladeStorm_Timer( Handle:timer, any:m_iClient )
{
    SetEntityRenderMode( m_iClient, RENDER_NORMAL );
    SetEntityRenderColor( m_iClient, 255, 255, 255, 255 );
    new ent = GetPlayerWeaponSlot( m_iClient, 2 );
    if ( ent > 0 && IsValidEntity( ent ) && IsValidEdict( ent ) ) TF2Attrib_SetByName( ent, "fire rate bonus", 1.0 );

    m_hTimers[m_iClient][m_hBladeStorm_TimerInterval] = INVALID_HANDLE;
}
public Action:m_tBladeStorm_TimerDuration( Handle:timer, any:m_iClient )
{
    if ( HasAttribute( m_iClient, 2, m_bBladeStorm_ATTRIBUTE ) ) {

        EmitSoundToClient( m_iClient, SOUND_NOTREADY );
        m_iIntegers[m_iClient][m_iBladeStorm_Stack] = 0;
    }

    m_hTimers[m_iClient][m_hBladeStorm_TimerDuration] = INVALID_HANDLE;
}
public Action:m_tBladeStorm_TimerCooldown( Handle:timer, any:m_iClient )
{
    if ( HasAttribute( m_iClient, _, m_bBladeStorm_ATTRIBUTE ) )
    {
        PrintHintText( m_iClient, "Custom: Blade Storm is ready." );
        EmitSoundToClient( m_iClient, SOUND_READY );
    }

    m_hTimers[m_iClient][m_hBladeStorm_TimerCooldown] = INVALID_HANDLE;
}
// Super Timer
