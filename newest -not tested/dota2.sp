...
#include <customweaponstf_orionstock_old>
#include <time>
...
new g_pDuelist_Target[MAXPLAYERS + 1] = -1;						// Stores the duelist (target)
new g_pDuelist_Caster[MAXPLAYERS + 1] = -1;						// Stores the duelist (caster)
...
new bool:m_bBurningSpear_ATTRIBUTE[MAXPLAYERS + 1][MAXSLOTS + 1];
new Float:m_flBurningSpear_Damage[MAXPLAYERS + 1][MAXSLOTS + 1];
new m_iBurningSpear_MaxStack[MAXPLAYERS + 1][MAXSLOTS + 1];
...
new Float:m_flBloodstone_ChargeDeath[MAXPLAYERS + 1][MAXSLOTS + 1];
new m_iBloodstone_ChargeStack[MAXPLAYERS + 1][MAXSLOTS + 1];
...
public OnPluginEnd()
{
	for ( new i = 1; i <= MaxClients; i++ )
	{
		if ( IsClientInGame( i ) )
		{
			...
			g_iLastWeapon[i]	= -1;
			g_pDuelist_Target[i]	= -1;
			g_pDuelist_Caster[i]	= -1;
		}
	}
}
...
public OnClientDisconnect( m_iClient )
{
	...
	g_pDuelist_Target[m_iClient]	= -1;
	g_pDuelist_Caster[m_iClient]	= -1;
}
...
// ====[ ON CLIENT PRETHINK ]==========================================
public OnClientPreThink( m_iClient ) OnPreThink( m_iClient );
	...
	if ( HasAttribute( m_iClient, _, m_bOverPower_ATTRIBUTE, true ) )
	{
		...
		if ( m_iButtons & IN_ATTACK2 == IN_ATTACK2 )
		{
			if ( m_hTimers[m_iClient][m_hOverPower_TimerCooldown] == INVALID_HANDLE )
			{
				...
				if ( m_hTimers[m_iClient][m_hOverPower_TimerDuration] == INVALID_HANDLE ) m_hTimers[m_iClient][m_hOverPower_TimerDuration] = CreateTimer( dur, m_tOverPower_Duration, m_iClient );
	...
	if ( HasAttribute( m_iClient, _, m_bEnchantTotem_ATTRIBUTE, true ) )
	{
		if ( m_iButtons & IN_ATTACK2 == IN_ATTACK2 )
		{
			if ( m_hTimers[m_iClient][m_hEnchantTotem_TimerCooldown] == INVALID_HANDLE )
			{
				...
				if ( m_hTimers[m_iClient][m_hEnchantTotem_TimerDuration] == INVALID_HANDLE ) m_hTimers[m_iClient][m_hEnchantTotem_TimerDuration] = CreateTimer( duration, m_tEnchantTotem_Duration, m_iClient );
				EmitSoundToClient( m_iClient, SOUND_RADIANCE );
	...
	if ( HasAttribute( m_iClient, _, m_bEnrage_ATTRIBUTE, true ) )
	{
		if ( m_iButtons & IN_RELOAD == IN_RELOAD )
		{
			if ( m_hTimers[m_iClient][m_hEnrage_TimerCooldown] == INVALID_HANDLE && HasAttribute( m_iClient, _, m_bFurySwipes_ATTRIBUTE ) )
			{
				...
				if ( m_hTimers[m_iClient][m_hEnrage_TimerDuration] == INVALID_HANDLE ) m_hTimers[m_iClient][m_hEnrage_TimerDuration] = CreateTimer( duration, m_tEnrage_Duration, m_iClient );
				EmitSoundToClient( m_iClient, SOUND_RADIANCE );
	...
	if ( HasAttribute( m_iClient, _, m_bInnerVitality_ATTRIBUTE ) ) {
		...
		if ( m_iButtons & IN_ATTACK2 == IN_ATTACK2 )
		{
			if ( m_hTimers[m_iClient][m_hInnerVitality_TimerCooldown] == INVALID_HANDLE )
			{
				...
				if ( m_hTimers[m_iClient][m_hInnerVitality_TimerDuration] == INVALID_HANDLE ) m_hTimers[m_iClient][m_hInnerVitality_TimerDuration] = CreateTimer( duration, m_tInnerVitality_Duration, m_iClient );
...
ATTRIBUTE_DUEL( m_iClient, &m_iButtons, &m_iSlot, &m_iButtonsLast )
{
	if ( HasAttribute( m_iClient, _, m_bDuel_ATTRIBUTE ) )
	{
		...
		// No need for g_pDuelist_Caster[x] as the attacker IS the client.
		if ( m_hTimers[m_iClient][m_hDuel_TimerDuration] != INVALID_HANDLE && IsValidClient( g_pDuelist_Target[m_iClient] ) && IsPlayerAlive( g_pDuelist_Target[m_iClient] ) && m_bBools[g_pDuelist_Target[m_iClient]][m_bIsDuel_On] )
		{
			...
			new m_iWeaponV = TF2_GetClientActiveWeapon( g_pDuelist_Target[m_iClient] );
			SetEntPropFloat( m_iWeaponV, Prop_Send, "m_flNextSecondaryAttack", GetGameTime()+duration );

			new Float:m_flPos1[3];
			GetClientEyePosition( m_iClient, m_flPos1 );
			new Float:m_flPos2[3];
			GetClientEyePosition( g_pDuelist_Target[m_iClient], m_flPos2 );
			...
			TeleportEntity( g_pDuelist_Target[m_iClient], NULL_VECTOR, m_flCam2, NULL_VECTOR );

			if ( m_iButtons & IN_BACK ) m_iButtons &= ~IN_BACK;
		}
		else if ( m_hTimers[m_iClient][m_hDuel_TimerDuration] == INVALID_HANDLE && IsValidClient( g_pDuelist_Target[m_iClient] ) )
		{
			m_bBools[g_pDuelist_Target[m_iClient]][m_bIsDuel_On] = false;
			g_pDuelist_Target[m_iClient] = -1;
		}
	}
...
PRETHINK_STACKREMOVER( m_iClient, &m_iButtons, &m_iSlot, &m_iButtonsLast )
{
	...
	if ( !HasAttribute( m_iClient, _, m_bBloodstone_ATTRIBUTE ) ) {
		if ( !g_hPostInventory[m_iClient] && IsPlayerAlive( m_iClient ) ) {
			m_iIntegers[m_iClient][m_iBloodstone_Charge] = 0;
			TF2Attrib_RemoveByName( m_iClient, "health regen" ); 
		}
	}
...
public Action:CW3_OnAddAttribute( m_iSlot, m_iClient, const String:m_sAttribute[], const String:m_sPlugin[], const String:m_sValue[], bool:m_bActive )
{
	...
	{
		new String:m_sValues[8][10];
		ExplodeString( m_sValue, " ", m_sValues, sizeof( m_sValues ), sizeof( m_sValues[] ) );

		m_iBloodstone_BaseCharge[m_iClient][m_iSlot]	= StringToInt( m_sValues[0] );
		m_iBloodstone_ChargeStack[m_iClient][m_iSlot]	= StringToInt( m_sValues[1] );
		m_flBloodstone_ChargeDeath[m_iClient][m_iSlot]	= StringToFloat( m_sValues[2] );
		m_flBloodstone_RegenPerCharge[m_iClient][m_iSlot]	= StringToFloat( m_sValues[3] );
		m_flBloodstone_BaseHeal[m_iClient][m_iSlot]		= StringToFloat( m_sValues[4] );
		m_flBloodstone_HealPerCharge[m_iClient][m_iSlot]	= StringToFloat( m_sValues[5] );
		m_flBloodstone_HealRadius[m_iClient][m_iSlot]	= StringToFloat( m_sValues[6] );
		m_flBloodstone_ChargeRadius[m_iClient][m_iSlot]	= StringToFloat( m_sValues[7] );
	 ...
	 *
	 * ---------------------------------------------------------------------- */
	else if ( StrEqual( m_sAttribute, "burning spear" ) )
	{
		new String:m_sValues[2][10];
		ExplodeString( m_sValue, " ", m_sValues, sizeof( m_sValues ), sizeof( m_sValues[] ) );

		m_flBurningSpear_Damage[m_iClient][m_iSlot]		= StringToFloat( m_sValues[0] );
		m_iBurningSpear_MaxStack[m_iClient][m_iSlot]	= StringToInt( m_sValues[1] );
...
public CW3_OnWeaponRemoved( m_iSlot, m_iClient )
{
	if ( IsValidClient( m_iClient ) )
	{
		if ( m_bHasAttribute[m_iClient][m_iSlot] )
		{
			...
			{
				m_iIntegers[m_iClient][m_iFervor_Stack]	= 0;
			...
			m_iBloodstone_ChargeStack[m_iClient][m_iSlot]	= 0;
			m_flBloodstone_ChargeDeath[m_iClient][m_iSlot]	= 0.0;
...
public Action:OnTakeDamage( m_iVictim, &m_iAttacker, &m_iInflictor, &Float:m_flDamage, &m_iType, &m_iWeapon, Float:m_iForce[3], Float:m_iPosition[3], m_iCustom )
{
		...
		&& !HasInvulnerabilityCond( m_iVictim ) )
	{
						...
						if ( m_bBurningSpear_ATTRIBUTE[m_iAttacker][m_iSlot] )
						{
							if ( m_iType & TF_DMG_AFTERBURN == TF_DMG_AFTERBURN ) {
								...
								if ( m_iIntegers[m_iVictim][m_iBSpear_Stack] > m_iBurningSpear_MaxStack[m_iAttacker][m_iSlot] )
									m_iIntegers[m_iVictim][m_iBSpear_Stack] = m_iBurningSpear_MaxStack[m_iAttacker][m_iSlot];
							}
						}
		...
		if ( IsValidClient( m_iAttacker ) )
		{
			 ...
			 *
			 * -------------------------------------------------- */
			if ( m_hTimers[m_iVictim][m_hDesolator_TimerDuration] != INVALID_HANDLE && !( m_iType & DOTA_DMG_BLADEMAIL ) && !( m_iType & DOTA_DMG_DISPERSION ) && !( m_iType & DOTA_DMG_OTHER ) )
		...
		&& m_iWeapon != -1 )
	{
		...
		//-//
			if ( m_bFervor_ATTRIBUTE[m_iAttacker][m_iSlot] )
			{
				...
				AttackSpeedLimit( m_iAttacker, m_iWeapon, m_iSlot, m_flAttackSpeed );
			}
		//-//
		...
		//-//
			if ( m_bDuel_ATTRIBUTE[m_iAttacker][m_iSlot] )
			{
				if ( m_hTimers[m_iAttacker][m_hDuel_TimerCooldown] == INVALID_HANDLE && m_bBools[m_iAttacker][m_bDuel_ReadyForIt] )
				{
					...
					g_pDuelist_Target[m_iAttacker] = m_iVictim;
					g_pDuelist_Caster[m_iVictim] = m_iAttacker;
				}
			}
		//-//
		...
		if ( HasAttribute( m_iClient, _, m_bStaticField_ATTRIBUTE ) )
		{
			for ( new i = 1; i <= MaxClients; i++ )
			{
				if ( i != m_iClient && IsClientInGame( i ) && IsPlayerAlive( i ) && GetClientTeam( i ) != GetClientTeam( m_iClient ) )
				{
					...
					{
						new damage = RoundToFloor( GetAttributeValueF( m_iClient, _, m_bStaticField_ATTRIBUTE, m_flStaticField_DamagePct ) * GetClientHealth( i ) );
						if ( damage >= 1 ) DealDamage( i, damage, m_iClient, TF_DMG_PREVENT_PHYSICS_FORCE|DOTA_DMG_OTHER );
					}
				}
			}
		}
	...
	if ( IsValidClient( m_iVictim ) )
	{
		...
		//-//
			...
			if ( HasAttribute( m_iVictim, _, m_bDuel_ATTRIBUTE ) ) // If the victim is the one having the attribute = The DUELIST won.
			{
				...
				{
					if ( IsValidClient( g_pDuelist_Target[m_iVictim] ) )
					{
						if ( IsValidClient( m_iKiller ) && m_iKiller == g_pDuelist_Target[m_iVictim] )
						{
						...
						ClearTimer( m_hTimers[g_pDuelist_Target[m_iVictim]][m_hDuel_TimerDuration] );
					}

					m_bBools[m_iKiller][m_bIsDuel_On] = false;
					g_pDuelist_Target[m_iVictim]	= -1;
					g_pDuelist_Caster[m_iKiller]	= -1;
					// HERE, g_pDuelist_Target[m_iVictim] IS THE WINNER	 == m_iKller.
					// AND, m_iVictim IS THE LOSER						== g_pDuelist_Caster[m_iKiller].
				}
			}
		//-//
			if ( m_bBools[m_iVictim][m_bIsDuel_On] && m_iVictim != m_iKiller ) // Duel is active and the victim didn't suicide.
			{
				if ( IsValidClient( g_pDuelist_Caster[m_iVictim] ) )
				{
					if ( IsValidClient( m_iKiller ) && m_iKiller == g_pDuelist_Caster[m_iVictim] )
					{
					...
					ClearTimer( m_hTimers[g_pDuelist_Caster[m_iVictim]][m_hDuel_TimerDuration] );
				}
				
				m_bBools[m_iVictim][m_bIsDuel_On] = false;
				g_pDuelist_Target[m_iKiller]	= -1;
				g_pDuelist_Caster[m_iVictim]	= -1;
				// HERE, m_iVictim IS THE LOSER							 == g_pDuelist_Target[m_iKiller].
				// AND, g_pDuelist_Caster[m_iVictim] IS THE WINNER		== m_iKiller.
			}
		//-//
			if ( HasAttribute( m_iVictim, _, m_bBloodstone_ATTRIBUTE ) )
			{
				...
				m_iIntegers[m_iVictim][m_iBloodstone_Charge] = RoundToCeil( m_iIntegers[m_iVictim][m_iBloodstone_Charge] * GetAttributeValueF( m_iVictim, _, m_bBloodstone_ATTRIBUTE, m_flBloodstone_ChargeDeath ) );
			}
		//-//
			for ( new i = 1; i <= MaxClients; i++ ) // Checks every client.
			{
				if ( IsClientInGame( i ) && IsPlayerAlive( i ) && GetClientTeam( i ) != GetClientTeam( m_iVictim ) && i != m_iKiller )
				{
					...
					if ( HasAttribute( i, _, m_bBloodstone_ATTRIBUTE ) )
					{
						if ( distance <= GetAttributeValueF( i, _, m_bBloodstone_ATTRIBUTE, m_flBloodstone_ChargeRadius ) ) {
							if ( m_iIntegers[i][m_iBloodstone_Charge] < GetAttributeValueI( i, _, m_bBloodstone_ATTRIBUTE, m_iBloodstone_ChargeStack )  )
		...
		if ( m_iKiller != m_iVictim )
		{
			...
			//-//
				if ( HasAttribute( m_iKiller, _, m_bBloodstone_ATTRIBUTE ) ) {
					if ( m_iIntegers[m_iKiller][m_iBloodstone_Charge] < GetAttributeValueI( m_iKiller, _, m_bBloodstone_ATTRIBUTE, m_iBloodstone_ChargeStack )  )
...
public Action:m_tDuel_Duration( Handle:timer, any:m_iClient )
{
	if ( HasAttribute( m_iClient, _, m_bDuel_ATTRIBUTE ) )
	{
		...
		m_bBools[g_pDuelist_Target[m_iClient]][m_bIsDuel_On] = false;
		g_pDuelist_Caster[g_pDuelist_Target[m_iClient]]	= -1;
		g_pDuelist_Target[m_iClient]						 = -1;

		// g_pDuelist_Target[m_iClient] = victim.
		// m_iClient = attacker.
	}
