...
new m_iSniperCombo_Miss[MAXPLAYERS + 1][MAXSLOTS + 1];
new m_iSniperCombo_Crit[MAXPLAYERS + 1][MAXSLOTS + 1];
...
new m_iBonkHealth_KnockBackRes[MAXPLAYERS + 1][MAXSLOTS + 1];
...
ATTRIBUTE_DAMAGERECEIVED( m_iClient, &m_iButtons, &m_iSlot, &m_iButtonsLast )
{
	if ( HasAttribute( m_iClient, _, m_bLevelUpSystem_DamageReceived_ATTRIBUTE, true ) )
	{
		...
		if ( m_flFloats[m_iClient][m_flTakeDamageCharge] < 500.0 ) {
			snd5[m_iClient] = false;
			TF2Attrib_RemoveByName( m_iWeapon, "attack projectiles" );
		}
		...
		{
			snd1[m_iClient] = true;
			EmitSoundToAll( SOUND_LVLUP1, m_iClient, SNDCHAN_WEAPON );
		...
		if ( m_flFloats[m_iClient][m_flTakeDamageCharge] >= 200.0 )
		{
			...
			{
				snd2[m_iClient] = true;
				EmitSoundToAll( SOUND_LVLUP2, m_iClient, SNDCHAN_WEAPON );
				TF2_AddCondition( m_iClient, TFCond_DefenseBuffMmmph, 2.0 ); 
			}
		}
		if ( m_flFloats[m_iClient][m_flTakeDamageCharge] >= 300.0 )
		{
			...
			{
				snd3[m_iClient] = true;
				EmitSoundToAll( SOUND_LVLUP3, m_iClient, SNDCHAN_WEAPON );
				TF2_AddCondition( m_iClient, TFCond_DefenseBuffMmmph, 2.0 ); 
			}
		}
		...
		{
			snd4[m_iClient] = true;
			EmitSoundToAll( SOUND_LVLUP4, m_iClient, SNDCHAN_WEAPON );
			TF2_AddCondition( m_iClient, TFCond_DefenseBuffMmmph, 2.0 ); 
		}
		if ( m_flFloats[m_iClient][m_flTakeDamageCharge] >= 500.0 )
		{
			TF2Attrib_SetByName( m_iWeapon, "attack projectiles", 1.0 );
...
ATTRIBUTE_ADDCONDALT( m_iClient, &m_iButtons, &m_iSlot, &m_iButtonsLast )
{
	if ( HasAttribute( m_iClient, _, m_bAddCondAltFire_ATTRIBUTE, true ) && m_iButtons & IN_ATTACK2 == IN_ATTACK2 )
	{
		...
		if ( !TF2_IsPlayerInCondition( m_iClient, TFCond:id ) )
		{
			...
			{
				DealDamage( m_iClient, ( hp >= 1.0 ? RoundToFloor( hp ) : RoundToFloor( GetClientHealth( m_iClient ) * hp ) ), m_iClient, TF_PREVENT_PHYSICS_FORCE );
...
ATTRIBUTE_SPEEDCLOAK( m_iClient, &m_iButtons, &m_iSlot, &m_iButtonsLast )
{
	if ( HasAttribute( m_iClient, _, m_bSpeedCloak_ATTRIBUTE ) ) { // Has he the attribute ? If he does, sets his new movement speed.
		if ( TF2_IsPlayerInCondition( m_iClient, TFCond_Cloaked ) || TF2_IsPlayerInCondition( m_iClient, TFCond_Stealthed ) ) // Is m_iClient invis ?
			SetClientMovementSpeed( m_iClient, GetClassBaseMovementSpeed( m_iClient ) * GetAttributeValueF( m_iClient, _, m_bSpeedCloak_ATTRIBUTE, m_flSpeedCloak_Multiplier ) );
		else SetClientMovementSpeed( m_iClient, GetClassBaseMovementSpeed( m_iClient ) );
	}
...
ATTRIBUTE_ELECTROSHOCK( m_iClient, &m_iButtons, &m_iSlot, &m_iButtonsLast )
{
	if ( HasAttribute( m_iClient, _, m_bElectroshock_ATTRIBUTE ) ) {
		if ( m_flFloats[m_iClient][m_flElectroshockEngine] >= GetEngineTime() - GetAttributeValueF( m_iClient, _, m_bElectroshock_ATTRIBUTE, m_flElectroshock_Duration ) )
		{
			...
			SetEntPropFloat( GetPlayerWeaponSlot( m_iClient, TFWeaponSlot_Primary ), Prop_Send, "m_flNextSecondaryAttack", GetGameTime()+2.0 );
		}
	}
...
public Action:CW3_OnAddAttribute( m_iSlot, m_iClient, const String:m_sAttribute[], const String:m_sPlugin[], const String:m_sValue[], bool:m_bActive )
{
	...
	else if ( StrEqual( m_sAttribute, "bonk health" ) )
	{
		...
		m_iBonkHealth_KnockBackRes[m_iClient][m_iSlot]	   = StringToInt( m_sValues[2] );
		m_flBonkHealth_OverHealBonusCap[m_iClient][m_iSlot]  = StringToFloat( m_sValues[3] );
	...
	else if ( StrEqual( m_sAttribute, "meet the sniper combo" ) )
	{
		...
		m_iSniperCombo_Miss[m_iClient][m_iSlot]	  = StringToInt( m_sValues[3] );
		m_iSniperCombo_Crit[m_iClient][m_iSlot]	= StringToInt( m_sValues[4] );
	...
	else if ( StrEqual( m_sAttribute, "meet the pyro combo" ) )
	{
		...
		m_iPyroCombo_Miss[m_iClient][m_iSlot]		= StringToInt( m_sValues[3] );
...
public CW3_OnWeaponRemoved( m_iSlot, m_iClient )
{
	if ( IsValidClient( m_iClient ) )
	{
		if ( m_bHasAttribute[m_iClient][m_iSlot] )
		{
			...
			m_iSniperCombo_Crit[m_iClient][m_iSlot]				= 0;
			...
			m_iBonkHealth_KnockBackRes[m_iClient][m_iSlot]	   = 0;
				...
				if ( HasAttribute( m_iVictim, _, m_bElectroshock_ATTRIBUTE ) )
				{
					if ( m_flFloats[m_iVictim][m_flElectroshock] >= 100.0 )
					{
						if ( TF2_DamageWillKill( m_iVictim, m_flDamage, true ) )
						{
							...
							TF2_RemoveCondition( m_iVictim, TFCond_Slowed );
							TF2_RemoveCondition( m_iVictim, TFCond_Zoomed );
							if ( TF2_GetClientActiveSlot( m_iVictim ) == 0 ) SetEntData( m_iWeapon, m_iMinigunOffset, 0, 4, false );
										...
										if ( distance <= m_flExplosiveDamage_Radius[m_iAttacker][m_iSlot] )
										{
											...
											if ( m_hSee != INVALID_HANDLE )
											{
												if ( !TR_DidHit( m_hSee ) )
												{
													...
													DealDamage( i, RoundToFloor( ( ( m_iExplosiveDamage_DamageMode[m_iAttacker][m_iSlot] == 1 ? m_flDamage : 1.0 ) * m_flExplosiveDamage_Damage[m_iAttacker][m_iSlot] ) * dmg_reduction ), m_iAttacker, ( m_iType & TF_DMG_CRIT ? TF_ALWAYSGIB|TF_DMG_BLAST|TF_DMG_CRIT|m_iType : TF_ALWAYSGIB|TF_DMG_BLAST|m_iType ), "pumpkindeath" );
					...
					//-//
						if ( m_bSniperCombo_ATTRIBUTE[m_iAttacker][m_iSlot] )
						{
							...
							if ( m_iIntegers[m_iAttacker][m_iSniperComboHit] <= 1 ) {
								...
								if ( m_iSniperCombo_Crit[m_iAttacker][m_iSlot] == 1 ) m_iType = TF_DMG_CRIT|m_iType;
		...
		&& IsValidClient( m_iAttacker ) )
	{
			...
			&& !HasInvulnerabilityCond( m_iVictim ) )
		{
			...
			//-//
				...
				{
					if ( m_flFloats[m_iVictim][m_flTakeDamageCharge] >= 400.0 && GetAttributeValueF( m_iVictim, _, m_bLevelUpSystem_DamageReceived_ATTRIBUTE, m_flLevelUpSystem_DamageReceived_CriticalDamageResistance, true ) == 0.0 ) {
					...
					else if ( m_flFloats[m_iVictim][m_flTakeDamageCharge] < 500.0 && m_flFloats[m_iVictim][m_flTakeDamageCharge] >= 400.0 ) m_flFloats[m_iVictim][m_flTakeDamageCharge] += ( m_flValue / 3.75 );

					if ( m_flFloats[m_iVictim][m_flTakeDamageCharge] > 500.0 ) m_flFloats[m_iVictim][m_flTakeDamageCharge] = 500.0;
					...
					//-//
						...
						if ( m_iType & TF_DMG_CRIT || IsCritBoosted( m_iAttacker ) )
						{
											...
											if ( distance <= m_flExplosiveCriticalDamage_Radius[m_iAttacker][m_iSlot] )
											{
												...
												if ( m_hSee != INVALID_HANDLE )
												{
													if ( !TR_DidHit( m_hSee ) )
													{
														...
														DealDamage( i, RoundToFloor( ( ( m_iExplosiveCriticalDamage_DamageMode[m_iAttacker][m_iSlot] == 1 ? m_flDamage : 1.0 ) * m_flExplosiveCriticalDamage_Damage[m_iAttacker][m_iSlot] ) * dmg_reduction ), m_iAttacker, TF_ALWAYSGIB|TF_DMG_BLAST|TF_DMG_CRIT|m_iType, "pumpkindeath" );
							...
							if ( HasAttribute( m_iVictim, _, m_bLevelUpSystem_DamageReceived_ATTRIBUTE, true ) && m_flFloats[m_iVictim][m_flTakeDamageCharge] >= 400.0 ) {
								m_flDamage *= GetAttributeValueF( m_iVictim, _, m_bLevelUpSystem_DamageReceived_ATTRIBUTE, m_flLevelUpSystem_DamageReceived_CriticalDamageResistance );
							}
						}
					//-//
...
public Action:TF2_CalcIsAttackCritical( m_iClient, m_iWeapon, String:m_strName[], &bool:m_bResult )
{
		...
		&& m_iWeapon != -1 )
	{
		...
		{
			if ( m_bSniperCombo_ATTRIBUTE[m_iClient][m_iSlot] && m_iSniperCombo_Miss[m_iClient][m_iSlot] == 1 )
			{
		...
		//-//
			if ( m_bPyroCombo_ATTRIBUTE[m_iClient][m_iSlot] && m_iPyroCombo_Miss[m_iClient][m_iSlot] == 1 )
			{
